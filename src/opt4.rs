use core::slice;
use std::{collections::HashMap, fs::File, ops::BitXor, sync::Mutex, thread};
...
fn split_buf(mut buf: &[u8], max_len: usize) -> Vec<&[u8]> {
    let mut bufs = Vec::with_capacity(buf.len().div_ceil(max_len));
    while !buf.is_empty() {
...
fn process_gpu<'a, F>(buf: &'a [u8], kernel_getter: F) -> Stations<'a>
where
...
fn process_cpu<'a>(buf: &'a [u8]) -> Stations<'a> {
    let mut stations = Stations::default();
    cpu_impl(buf, &mut stations.inner);
    stations
}

fn merge_stations<'a>(base: &Mutex<Stations<'a>>, update: Stations<'a>) {
    let base = &mut base.lock().unwrap().inner;
    for (name, station) in update.inner.into_iter() {
        if let Some(base_station) = base.get_mut(name) {
            base_station.merge(&station);
        } else {
            base.insert(name, station);
        }
    }
}
    ...
    F: FnOnce(&metal::Device) -> metal::Function,
{
    ...
    // let (gpu_buf, cpu_buf) = split_buf_once(buf, buf.len());
    ...
    let stations = Mutex::new(Stations::default());
    thread::scope(|s| {
        s.spawn(|| merge_stations(&stations, process_gpu(gpu_buf, kernel_getter)));
        for chunk in cpu_chunks {
            s.spawn(|| merge_stations(&stations, process_cpu(chunk)));
        }
    });

    println!("{}", stations.into_inner().unwrap());
}
